...
## 6.3 심화: 난이도 스케일링과 로봇 이식 (2026-09-04 배치)

- **난이도 한계선**: 계단 12 cm(medium)부터 전 알고리즘 성공률 0 % — blind
  정책의 한계는 계단 5→12 cm, 요철 ±5→±8 cm 사이. 커리큘럼 학습 필요성의 실증.
- **보수성의 상한**: easy~medium 험지 1위였던 SAC가 험지 hard(±8 cm)에서 최하
  (0.090 m/s) — 극한 난이도에서 보수적 정책은 전진 포기로 수렴. PPO/TRPO는
  전복을 감수하며 0.56~0.60 유지.
- **로봇 이식(ANYmal-C, A1 튜닝 보상 그대로)**: 최고 성적 PPO 평지 0.855/성공
  67 %로 전반적 저성능. TD3는 전 지형 0.08~0.20 m/s로 사실상 실패, SAC는 저속
  이지만 전복 최저(6~26/min) 유지. 질량·토크 스케일에 대한 보상 정규화 없이는
  이식이 실패함을 정량화 — 로봇별 보상 스케일링이 후속 과제.
