...
def main() -> None:
    ...
    running_cmds = subprocess.run(
        ["pgrep", "-af", "scripts/train.py"], capture_output=True, text=True
    ).stdout
    ...
    for i, cmd in enumerate(jobs):
        ...
        if st.get("state") == "done":
            ...
            is_run = any(
                all(t in line for t in toks) and (lvl or "terrain_level" not in line)
    ...
    print(
        f"  {bar(len(done) / n)}  {BOLD}{len(done)}/{n}{END} "
