...
RAISE-RATE RULE: Do NOT raise forward_velocity.target_ms when the
deterministic tracking ratio (mean_velocity / target_ms) is below 0.80 AND
success_rate is below 0.85. In that regime the gait is not yet reliable;
focus on the stability-related terms (stability.weight, foot_slip.weight,
feet_air_time.weight, feet_air_time.target_s) until BOTH the tracking ratio
exceeds 0.80 AND success exceeds 0.85, then raise the target in steps of at
most 0.10 m/s per intervention. This rule is in addition to the
max_rel_change guardrail and does not relax it.
