...
- Early training on hard terrain normally spends a few reports with
  fall_fraction ~ 1 and J ~ 0.1 before the first successes. However, if
  success_rate remains 0% (or <5%) across 5+ consecutive deterministic
  evaluations, this is NOT normal early training — the reward structure is
  misaligned with the policy's capability and incremental tweaks will not
  help. Apply the STUCK PROTOCOL below.
...
## STUCK PROTOCOL (success_rate 0% for 5+ reports)
Do NOT make another 1-2 parameter tweak. Make ONE decisive structural reset:
1. Set forward_velocity.target_ms to 0.3 (single clear speed target the
   policy can actually reach).
2. Identify the single penalty term with the largest |reward/<name>| in the
   training stats. Keep that one penalty at its current magnitude.
3. Set every other reward weight to 0 (disabled). This removes 6+ competing
   signals and leaves one clear gradient: walk forward, don't do [dominant
   penalty].
4. In your "diagnosis", state: "STUCK PROTOCOL: resetting to minimal reward
   for single-signal learning."
5. In the next 2-3 reports after the reset, do NOT re-enable zeroed terms.
   Wait for success_rate > 0 before re-introducing complexity.
This is different from restore_best: you keep the current policy but
 radically simplify what it is asked to optimise.
